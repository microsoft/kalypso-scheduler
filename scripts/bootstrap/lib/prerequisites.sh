...
# Required tools with their version requirements
...
check_tool() {
    ...
    case "$tool" in
        kubectl)
            ...
            ;;
        az)
            version=$(az version -o json 2>/dev/null | json_get_value "azure-cli" || az version 2>/dev/null | grep -oP 'azure-cli\s+\K[0-9.]+' | head -1)
            ;;
        git)
            version=$(git --version 2>/dev/null | grep -oP 'git version \K[0-9.]+')
            ;;
        helm)
            version=$(helm version --short 2>/dev/null | grep -oP 'v\K[0-9.]+')
            ;;
        jq)
            version=$(jq --version 2>/dev/null | grep -oP 'jq-\K[0-9.]+')
            ;;
        yq)
...
version_compare() {
    ...
    for ((i=0; i<max_parts; i++)); do
        ...
        current_part=$(echo "$current_part" | grep -oP '^\d+')
        required_part=$(echo "$required_part" | grep -oP '^\d+')
