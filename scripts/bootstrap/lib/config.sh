...
prompt_cluster_config() {
    ...
    if [[ "$CREATE_CLUSTER" != "true" && "$USE_EXISTING_CLUSTER" != "true" ]]; then
    ...
    if [[ "$USE_EXISTING_CLUSTER" == "true" ]]; then
        # Prompt for existing cluster details
...
validate_configuration() {
    ...
    # Validate cluster mode is set
    if [[ "$CREATE_CLUSTER" != "true" && "$USE_EXISTING_CLUSTER" != "true" ]]; then
        log_error "Must specify either --create-cluster or --use-cluster" "config"
        valid=false
    fi
